...
const real=load(),test=load('?test-draft=1402785901897109504');
...
for(const key of ['fd26-draft','fd26-ord','fd26-slot','fd26-intel','fd26-tendency','fd26-practice','fd26-practice-backup','fd26-blocked','fd26-sleeper','fd26-sync-on']){
 ...
 assert.equal(test.draftStorageKey(key),'fd26-test-1402785901897109504:'+key);
}
...
assert.match(real.assigned[0],/test-draft=1402785901897109504/);
...
for (const [name, key, getter, setter, realValue, testValue] of [
 ...
 const prefix='fd26-test-1402785901897109504:';
 ...
 const isolated=run('?test-draft=1402785901897109504');
 ...
 assert.equal(run('?test-draft=1402785901897109504')[getter]('Player'),testValue);
}
