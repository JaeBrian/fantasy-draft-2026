...
The Weekly lineup tab fetches the same real league independently of draft-room state. It refreshes once a minute while mounted, times out stalled requests after 15 seconds, backs off on errors, and keeps the last successful snapshot visibly marked. It is not a background scheduled job. Player metadata is fetched only once rosters contain players and cached in memory for a day.
