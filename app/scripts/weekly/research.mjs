...
export function combineResearch(dataset,{environment,coverage,stats},previous=null,now=Date.now()){
 ...
 for(const p of Object.values(players)){
  ...
  else if(p.positions.some(x=>['WR','TE'].includes(x)))notes.push('Individual WR/CB route assignments remain unverified. Team defense and observed receiving metrics are available; defender reputation does not change points.');
