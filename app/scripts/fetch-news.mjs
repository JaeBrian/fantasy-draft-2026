...
const playerResponse = await fetch("https://api.sleeper.app/v1/players/nfl", { signal: AbortSignal.timeout(15000) });
if (!playerResponse.ok) throw new Error(`Player catalog HTTP ${playerResponse.status}; keeping the previous news file`);
...
const missingIds = [...board].filter(name => !idOf[name]);
if (!board.size || missingIds.length) throw new Error(`Incomplete player lookup; keeping the previous news file. Missing: ${missingIds.join(', ')}`);
...
for (const p of Object.values(players)) {
  if (!skillPosition(p) || !p.team || !p.active) continue;
...
const out = {};
let done = 0, withNews = 0, seen = 0, dropped = 0;
const errors = [];
...
for (let i = 0; i < entries.length; i += CONC) {
  await Promise.all(entries.slice(i, i + CONC).map(async ([name, pid]) => {
    try {
      const items = await fetchPlayerNews(pid);
  ...
  if (errors.length) throw new Error(`Incomplete news refresh; keeping the previous file.\n${errors.join('\n')}`);
...
export const NEWS_FETCHED_AT = ${JSON.stringify(new Date().toISOString())};
export const NEWS: Record<string, NewsItem[]> = ${JSON.stringify(out)};
`;
writeFileSync(app + "src/news.ts", ts);
console.log(`\nwrote src/news.ts — ${withNews} players with sourced news, 0 errors, ${(ts.length / 1024).toFixed(0)} KB`);
