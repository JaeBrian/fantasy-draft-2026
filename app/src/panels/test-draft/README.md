...
Polling matches the real draft room: 10 seconds after each response, including empty drafts and errors. Empty live snapshots clear the board after a Sleeper draft reset.
...
The Sleeper draft URL in Room settings is editable and saved separately for test mode. Paste a new URL to reconnect. Select the matching seat; when the new draft has picks, its snapshot replaces the previous board. An empty live draft clears prior marks automatically.
